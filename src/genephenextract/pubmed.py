...
class PubMedClient:
    ...
    def fetch_pmc_full_text(self, pmcid: str) -> Optional[str]:
        ...
        supplementary_sections = _gather_supplementary_texts(xml_root, pmcid, timeout=self.timeout)
        if supplementary_sections:
            text_parts.append("## Supplementary Materials")
            text_parts.extend(supplementary_sections)
            logger.info(
                "Appended %d supplementary materials for %s", len(supplementary_sections), pmcid
            )
